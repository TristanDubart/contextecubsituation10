...
#
#VERSION : 1.1
#COMMENTS : Demande à l'utilisateur la zone DNS qu'il veut créer.Et si enregistrement de type A ou de type CNAME.
...
if (-not (Get-DnsServerZone -Name $zoneName -ErrorAction SilentlyContinue)) {
    ...
    # Demander à l'utilisateur s'il veut ajouter un enregistrement de type A ou de type CNAME
    $recordType = Read-Host "Voulez-vous ajouter un enregistrement de type A ou de type CNAME ? (A/CNAME)"
    switch ($recordType.ToLower()) {
        "a" {
            $recordName = Read-Host "Entrez le nom de l'enregistrement (par exemple : www)"
            $recordIPAddress = Read-Host "Entrez l'adresse IP associée à cet enregistrement"
            Add-DnsServerResourceRecordA -Name $recordName -ZoneName $zoneName -IPv4Address $recordIPAddress
            Write-Host "Enregistrement de type A ajouté avec succès pour $recordName dans la zone $zoneName."
        }
        "cname" {
            $recordName = Read-Host "Entrez le nom de l'enregistrement (par exemple : www)"
            $recordAlias = Read-Host "Entrez le nom canonique alias pour cet enregistrement"
            Add-DnsServerResourceRecordCName -Name $recordName -ZoneName $zoneName -HostNameAlias $recordAlias
            Write-Host "Enregistrement de type CNAME ajouté avec succès pour $recordName dans la zone $zoneName."
        }
        default {
            Write-Host "Type d'enregistrement non reconnu."
        }
    }
} else {
